...
@section('authContentOne')
    <div class="card border-dark mb-2">
        <div class="card-body text-dark mt-0">
            <div class="row mb-2">
                <p>
                    @if (Auth::user()->hasUserPermission(["OAGPSMP02"]) == true)
                        <a href="{{ route("oil.and.gas.pump.supplier.create",["oagpSlug" => $oilAndGasPump->slug]) }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i> Create supplier</a>
                    @endif
                ...
                <div class="card-body" id="dataTableGrid">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @forelse ($suppliers as $perSupplierIndex => $perSupplier)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if (Auth::user()->hasUserPermission(["OAGPSMP04"]) == true)
                                            ...
                                            @if (Auth::user()->hasUserPermission(["OAGPSMP05"]) == true)
                                                ...
                                                <div class="modal fade" id="{{$perSupplier->slug}}DeleteConfirmationModal" tabindex="-1">
                                                    <div class="modal-dialog modal-lg">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h1 class="modal-title fs-5">{{ $perSupplier->name }} delete confirmation.</h1>
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <form action="{{ route("oil.and.gas.pump.supplier.delete",["oagpSlug" => $oilAndGasPump->slug,"sSlug" => $perSupplier->slug]) }}" method="POST">
                                                                    @csrf
                                                                    @method("DELETE")
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5">
                                            <b class="d-flex justify-content-center text-warning">No supplier found.</b>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
...
@push("onPageExtraScript")
    <script>
        ...
        function dataTableLoad(parameterString){
            var routeUrl = "{{ route('oil.and.gas.pump.supplier.index',['oagpSlug'=> $oilAndGasPump->slug]) }}"
